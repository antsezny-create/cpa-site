...
function loadFirebaseClients() {
  db.collection("clients").orderBy("createdAt", "desc").onSnapshot(function(snapshot) {
    ...
    snapshot.forEach(function(doc) {
      ...
      item.onclick = function() { openFirebaseConversation(doc.id, name, initials, color, client.type || "Individual", this); };
      item.innerHTML =
...
function openFirebaseConversation(clientId, name, initials, color, type, element) {
  ...
  thread.innerHTML = '<div style="text-align: center; color: #6B7280; padding: 40px; font-size: 13px;">Loading messages...</div>';
  ...
  try {
    messageUnsubscribe = db.collection("messages")
      ...
      .onSnapshot(function(snapshot) {
        ...
        // Sort messages by timestamp in JavaScript instead
        let messages = [];
        snapshot.forEach(function(doc) {
          let msg = doc.data();
          msg._id = doc.id;
      ...
      thread.innerHTML = '<div style="text-align: center; color: #EF4444; padding: 40px; font-size: 13px;">Error: ' + error.message + '</div>';
    });
  } catch(err) {
    thread.innerHTML = '<div style="text-align: center; color: #EF4444; padding: 40px; font-size: 13px;">Caught error: ' + err.message + '</div>';
  }
}
